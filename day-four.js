...
let passports = testData2.reduce((acc, el) => {
    if (el === '') {
        
        passportIdx += 1;
        // console.log('index ==================>', passportData)
        acc.push({});
        return acc;
    }
...
//if '' create new obj, increment idx, create new obj?
// split el by space
// split new els by ":"
// add to obj
...
let pCheck = passports.map(obj =>{
    if (obj.hasOwnProperty('byr') && obj.hasOwnProperty('iyr') && obj.hasOwnProperty('eyr') && obj.hasOwnProperty('hgt') && obj.hasOwnProperty('hcl') && obj.hasOwnProperty('ecl') && obj.hasOwnProperty('pid')) {
        return true;
    }
...
let pCheck2 = passports.map(obj =>{
    ...
    let hcl = obj['hcl']
    let ecl = obj['ecl']
    eyeArr = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
    let pid = obj['pid']
    console.log(eyr)
    ...
    if (hgt.slice(-2) === 'in') {
        if (+hgt.slice(0,-2) <= 59 || +hgt.slice(0,-2) >= 76) {
            console.log('hgt in', hgt)
            return false;
        }
    }

    if (hgt.slice(-2) === 'cm') {
        if (+hgt.slice(0,-2) <= 150 || +hgt.slice(0,-2) >= 193) {
            console.log('hgt cm', hgt)
            return false;
        }
    }
...
console.log(pCheck2)
console.log(pCheck2.filter(x => x === true).length);