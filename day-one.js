...
let hashTwoSums = (arr, sum) => {
    ...
    for (let i in arr) {
        ...
        hash[arr[i]] = i;
        console.log('hash:', hash, 'nums:', nums, 'addend:', addend)
    }

    return nums
}

let hashThreeSums = (arr, sum) => {

    for (let i in arr) {
        let addends = hashTwoSums(arr, sum - arr[i]);
        if (addends.length) {

            return [arr[i], ...addends].reduce((acc, curr) => acc * curr);
        }
    }
}

// console.log(numList);
console.log(hashTwoSums(numList, 2020));
console.log(hashThreeSums(numList, 2020));
